...
app.secret_key = os.environ.get('SECRET_KEY', secrets.token_hex(16))  # Use environment variable or generate a random key
app.config['SESSION_TYPE'] = 'filesystem'
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_SAMESITE'] = None  # None is required for cross-origin requests
app.config['SESSION_COOKIE_SECURE'] = os.environ.get('ENVIRONMENT', 'development') == 'production'  # True in production
...
def serve_static(path):
    # Special handling for label.html to inject environment variables
    if path == 'label.html':
        try:
            with open('label.html', 'r') as f:
                content = f.read()

            # Get environment variables using the helper function
            domains = get_api_domains()

            # Replace placeholders with actual values
            content = content.replace('{{API_DOMAIN}}', domains['api_domain'])
            content = content.replace('{{SOCKET_DOMAIN}}', domains['socket_domain'])
            content = content.replace('{{TASKS_API_DOMAIN}}', domains['tasks_api_domain'])

            return content, 200, {'Content-Type': 'text/html'}
        except Exception as e:
            print(f'Error serving label.html with environment variables: {str(e)}')
            # Fall back to normal serving if there's an error
...
def proxy_get_user_tasks():
    try:
        ...
        # Get the tasks API domain from environment variables
        domains = get_api_domains()
        tasks_api_domain = domains['tasks_api_domain']
        ...
        response = requests.post(
            f'{tasks_api_domain}/get-user-tasks',
...
def proxy_get_bounding_boxes():
    try:
        ...
        response = requests.get(
            f'{tasks_api_domain}/get/{filename}',
...
# Get environment variables for API domains
def get_api_domains():
    """Get all API domains from environment variables"""
    api_domain = os.getenv('API_DOMAIN', 'http://localhost:5001')
    socket_domain = os.getenv('SOCKET_DOMAIN', api_domain)
    tasks_api_domain = os.getenv('TASKS_API_DOMAIN', 'http://192.168.1.98:5000')
    ...
    return {
        ...
        'tasks_api_domain': tasks_api_domain,
        'port': port
    }
...
if __name__ == "__main__":
    ...
    print(f"Socket Domain: {domains['socket_domain']}")
    print(f"Tasks API Domain: {domains['tasks_api_domain']}")
    print(f"Port: {domains['port']}")

    # Run the Flask app
    # Set debug mode based on environment
    debug_mode = os.getenv('ENVIRONMENT', 'development') != 'production'
    app.run(host='0.0.0.0', port=domains['port'], debug=debug_mode)
